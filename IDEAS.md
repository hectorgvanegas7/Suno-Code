...
## 1b. QA de letra — Capa 3 ("El Guardia", IMPLEMENTADA 2026-07-12)
...
*   **Estado:** la tercera capa ya existe como **"El Guardia"** (`lib/ollama-guardia.js`): segunda opinión INDEPENDIENTE vía LLM local (Ollama, default `qwen3:14b`, configurable con `GUARDIA_MODEL` — ej. `qwen3:8b` si el 14b resulta lento). En vez del proofreading puro que se había imaginado acá (Haiku), juzga cómo está ARMADA la canción — coherencia/rima/tono/fidelidad a la encuesta/gancho, 1-10 cada una + problemas concretos por sección — que es lo que ni el diccionario ni LanguageTool pueden ver, y que hasta ahora solo se autoevaluaba el propio modelo generador. Corre en `run.js` DESPUÉS de que la letra ya pasó QA completo, **puramente informativo** (nunca bloquea ni gasta reintentos, mismo criterio que CLAP/NISQA): consola + `state.json` (`guardia`) + `logs/guardia-feedback.jsonl` para calibrar contra el QA humano real antes de decidir si algún día sube a gate.
*   **Pendiente:** juntar veredictos reales en `logs/guardia-feedback.jsonl` y compararlos contra REDOs/feedback de QC. Si el Guardia demuestra atrapar problemas reales que las otras capas dejan pasar, recién ahí considerar darle poder de gate/reintento.
