...
// ─── Decisión PURA del Auto-Submit (test/flow-helpers.test.js) ───────────────
// Única autoridad sobre si el Auto-Submit puede disparar. Cortocircuitos, en
// orden de peligro:
//   'submit-already-clicked' — un intent de submit de ESTA canción ya registra
//       clickedAt (esta corrida u otra anterior que murió a mitad): repetir el
//       click sería un DOBLE Submit a QA. Jamás.
//   'no-upload' — sin MP3 nuevo confirmado en esta corrida, submitear mandaría
//       a QA lo que hubiera antes (en un REDO, la versión vieja rechazada).
//   'too-early' — el timer anti-bot (26-31 min) todavía no llegó.
//   { go: true } — todo verde.
function shouldAutoSubmit({ elapsedMin, autoSubmitMinutes, uploadConfirmed, submitIntent, songId }) {
  if (submitIntent && submitIntent.songId === songId && submitIntent.clickedAt) {
    return { go: false, reason: 'submit-already-clicked' };
  }
  if (!uploadConfirmed) {
    return { go: false, reason: 'no-upload' };
  }
  if (!(elapsedMin >= autoSubmitMinutes)) {
    return { go: false, reason: 'too-early' };
  }
  return { go: true };
}

module.exports = { FLOW_URL, waitForLyrics, enterFlowAndEnsureAssignment, shouldAutoSubmit };
