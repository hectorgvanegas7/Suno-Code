...
async function patchSongLines(parsedJson, patchableIssues) {
  ...
  const prompt = `Esta letra de canción tiene problemas mecánicos, poéticos o de contenido en algunas líneas específicas, reportados por un auditor. NO reescribas nada más — cambiá ÚNICAMENTE las líneas listadas abajo. Si el motivo indica redundancia, rima pobre, error lógico o gramatical, reescribe la línea para que sea poética, fluida y tenga sentido, preservando la métrica y la rima general. Cada sección debe mantener exactamente la misma cantidad de líneas que tiene ahora.
