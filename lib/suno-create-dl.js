...
async function saveDownload(download, destPath, label, timeoutMs = DOWNLOAD_WAIT_TIMEOUT_MS) {
  // saveAs() no tiene timeout propio — si la descarga se estancara a mitad
  // de camino quedaría colgado para siempre. Mismo techo de 8 min que tenía
  // el watcher de filesystem que reemplaza, para no perder esa garantía.
  await Promise.race([
    download.saveAs(destPath),
    new Promise((_, reject) => setTimeout(
      () => reject(new Error(`Timeout ${timeoutMs}ms guardando la descarga de la versión ${label}.`)),
      timeoutMs
    )),
  ]);
  const stat = fs.statSync(destPath);
  console.log(`  ✅ Versión ${label} descargada: ${path.basename(destPath)} (${Math.round(stat.size / 1024)} KB)`);
  return destPath;
}
...
async function createAndDownload({ sunoDir = SUNO_DIR } = {}) {
  ...
  try {
    ...
    // a la vez): para cada versión, clickear "MP3 Audio" y esperar a que
    // Chrome confirme el INICIO de la descarga (segundos, no minutos — ver
    // clickDownloadMp3). Si el click automático falla del todo, el fallback
    // manual corre EN ESTE MISMO paso (nunca en paralelo entre versiones —
    // pauseForHumanInteraction comparte el listener de stdin).
    //
    // Fase B (paralela): guardar cada Download confirmado a su destino con
    // saveAs(). Cada Download ya es una referencia inequívoca a una descarga
    // concreta (API nativa de Playwright, 2026-07-04 — ver LESSONS.md), así
    // que no hay ninguna carpeta compartida que vigilar ni riesgo de que A y
    // B se confundan entre sí, a diferencia del viejo watcher de filesystem.
    const cleanTitle = verify.titulo.replace(/[<>:"\/\\|?*]+/g, '').trim();
    const versionLabels = ['A', 'B'];

    const plans = []; // { label, destPath, download }
    for (let i = 0; i < readyCards.length; i++) {
      ...
      try {
        ...
        // Fallo estructural (card/⋯ desaparecieron) — no hay nada que clickear.
      ...
      plans.push({ label: versionLabel, destPath, download });
      await page.waitForTimeout(500); // pequeño margen para que el menú se cierre antes de tocar la próxima card
    }

    const downloadedPaths = [];
    const settled = await Promise.allSettled(
      plans.filter((p) => p.download).map((p) =>
        saveDownload(p.download, p.destPath, p.label).then((savedPath) => ({ label: p.label, path: savedPath }))
      )
    );
    ...
    for (const p of plans.filter((p) => !p.download)) {
