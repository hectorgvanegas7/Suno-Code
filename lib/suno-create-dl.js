...
async function downloadVia3DotMenu(page, href, label, sunoDir, destPath, deadlineMs = DOWNLOAD_WAIT_TIMEOUT_MS) {
  ...
  try {
    ...
    if (!clicked) {
      throw new Error(
        `Timeout esperando que "MP3 Audio" esté disponible para versión ${label} ` +
        `(${Math.round(deadlineMs / 60000)} min). La canción quizá no terminó de renderizar.`
      );
    }
  ...
  try {
    ...
    if (fresh.length === 0) {
      ...
      throw new Error(
        ...
        `${CREATE_CARDS_TIMEOUT_MS / 1000}s (dos intentos). Screenshot: ` +
