...
async function waitForGeneration(page, newHrefSet, normTitle, expectedCount, timeoutMs = GENERATION_TIMEOUT_MS) {
  ...
  while (Date.now() - startTime < timeoutMs) {
    ...
    if (candidates.length === 0) {
      emptyPolls++;
      if (emptyPolls >= 40) { // 40 * 3s = 120s (2 min)
        throw new Error(
...
async function pollForNewFile(sunoDir, cleanTitle, downloadStartTime, timeoutMs) {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      ...
      // Ignorar errores temporales de lectura
    }
...
async function clickDownloadMp3(page, href, label, deadlineMs, { sunoDir = null, cleanTitle = null, downloadStartTime = null } = {}) {
  ...
  // Escuchar el evento nativo de descarga ANTES de intentar el click.
  // Como fallback para cuando Playwright se conecta sin interceptación de descargas (noDefaults: true),
  // también polleamos de forma paralela la carpeta de descargas buscando el nuevo archivo.
  const playwrightDownloadPromise = page.waitForEvent('download', { timeout: DOWNLOAD_START_CONFIRM_TIMEOUT_MS }).catch(() => null);
  const pollDownloadPromise = pollForNewFile(sunoDir, cleanTitle, downloadStartTime, DOWNLOAD_START_CONFIRM_TIMEOUT_MS);
