...
//
// Llena: Date (A), Total Songs (B), Title (E), Song ID (F).
// Deja vacías: Total Time (C), Time (D), Remarks (G), Screenshot (H) — esas
// las llena Gabo a mano. NUNCA toca C ni D.
//
...
function parseSongFile(content) {
  const tituloMatch = content.match(/\*\*Título:\*\*\s*(.+)/i);
...
async function logSongToSheet({ songPath = DEFAULT_SONG_PATH, log = console.log } = {}) {
  if (!fs.existsSync(songPath)) {
    throw new Error('song.txt no encontrado. Corré primero el flujo de generación.');
  }
  if (!fs.existsSync(CREDENTIALS_PATH)) {
    throw new Error('google-credentials.json no encontrado en la carpeta del proyecto.');
  }
