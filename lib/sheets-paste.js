...
// Pone el PNG en el portapapeles como imagen real (no como referencia a
// archivo) para que un Ctrl+V/Cmd+V en cualquier app lo pegue como imagen —
// igual que copiar una imagen desde un visor y pegarla.
// Windows: PowerShell + Clipboard.SetImage (requiere -sta: tira una excepción
// de threading sin modo STA). macOS: osascript + NSPasteboard vía "set the
// clipboard to ... as TIFF picture".
function copyImageToClipboard(localPngPath) {
  try {
    if (process.platform === 'darwin') {
      const escapedPath = localPngPath.replace(/"/g, '\\"');
      execFileSync('osascript', ['-e', `set the clipboard to (read (POSIX file "${escapedPath}") as TIFF picture)`], { stdio: 'ignore' });
      return true;
    }
