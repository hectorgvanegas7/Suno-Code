...
// Verifica que el puerto no esté en uso. Si lo está, aborta el script
// para evitar un crash feo al intentar instanciar Chromium.
async function ensurePortIsFree(port) {
  if (await isPortUp(port)) {
    throw new Error(
      `El puerto ${port} está bloqueado/ocupado.\n` +
...
async function pauseForHumanInteraction(reason, options = {}) {
  ...
  // Si ya se abortó, salir de inmediato
  if (options.abortSignal && options.abortSignal.aborted) {
    return;
  }

  // Pausar hasta que el usuario presione ENTER o se cancele la señal
...
module.exports = { safeClick, identifyBlocker, isClickable, clickByText, setSliderValue, expandIfCollapsed, withReloadRetry, connectToSunoTab, isLoggedIn, isPortUp, ensurePortIsFree, pauseForHumanInteraction };
