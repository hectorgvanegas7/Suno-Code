...
async function pauseForHumanInteraction(reason, options = {}) {
  ...
  try {
    return await waitForEnterKey(options.abortSignal, timeoutMs);
  } catch (e) {
    if (e instanceof HumanTimeoutError) {
      console.error(`\n⏱️ ${e.message}\n`);
...
async function confirmToContinue(summary, { nextAction = 'continuar', abortSignal = null, timeoutMs = null } = {}) {
  ...
  await waitForEnterKey(abortSignal, effectiveTimeoutMs);
  console.log('✅ Confirmado — continuando.\n');
}

module.exports = { safeClick, identifyBlocker, isClickable, clickByText, setSliderValue, expandIfCollapsed, withReloadRetry, connectToSunoTab, connectToFlowTab, isLoggedIn, isPortUp, ensurePortIsFree, pauseForHumanInteraction, confirmToContinue, waitForEnterKey, HumanTimeoutError, defaultHumanTimeoutMs };
