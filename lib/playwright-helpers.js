...
const { notify, notifyWithReplyActions, waitForNtfyReply, notifyAttachment, newRequestId } = require('./ntfy');
...
async function identifyBlocker(page, locator) {
  try {
    ...
    const info = await page.evaluate(([x, y]) => {
      ...
      if (el.className) {
        let cls = typeof el.className === 'string' ? el.className : (el.className.baseVal || '');
        if (cls) parts.push(`.${cls.trim().split(/\s+/).slice(0, 2).join('.')}`);
      }
...
async function connectToSunoTab(chromium, debugPort = 9333) {
  // { noDefaults: true } — sin esto, Playwright inyecta sus manejadores/rutas
  // por default al conectar sobre CDP, lo que puede chocar con la sesión que
  // ya está viva en un Chrome de larga duración y tirar "Protocol error"
  // (bug real, confirmado en vivo). Ver LESSONS.md: "CDP lifecycle pattern".
...
// Abort REMOTO explícito (botón 🛑 en la notificación ntfy, 2026-07-14).
// Extiende HumanTimeoutError A PROPÓSITO: todos los callers existentes ya
// tratan HumanTimeoutError como "abandonar esta canción y seguir con la
// próxima" — un abort remoto quiere exactamente ese comportamiento, sin
// tener que tocar cada catch del repo. La diferencia es solo semántica
// (decisión humana inmediata vs nadie respondió) y queda en name/message.
class HumanAbortError extends HumanTimeoutError {
  constructor(message) {
    super(message);
    this.name = 'HumanAbortError';
  }
}
...
//
// timeoutMs (opcional): sin esto, espera para SIEMPRE — el comportamiento de
// siempre para una corrida atendida. Con `--loop` (pensado para dejar el
// pipeline corriendo toda la noche sin nadie mirando), start-flow.js setea
// CANCIONETERNA_HUMAN_TIMEOUT_MS en el entorno (heredado por los scripts hijo
// que spawnea) para que esta espera NUNCA sea infinita quedando la cola
// entera trabada detrás de una sola canción rota — ver LESSONS.md.
function waitForEnterKey(abortSignal = null, timeoutMs = null) {
  if (abortSignal && abortSignal.aborted) return Promise.resolve({ timedOut: false });
  ...
  return new Promise((resolve, reject) => {
    ...
    const onTimeout = () => {
      cleanup();
      reject(new HumanTimeoutError(`Pasaron ${Math.round(timeoutMs / 60000)} min sin respuesta humana — se abandona esta acción en vez de esperar para siempre.`));
    };
...
function defaultHumanTimeoutMs() {
  const fromEnv = parseInt(process.env.CANCIONETERNA_HUMAN_TIMEOUT_MS, 10);
  return Number.isFinite(fromEnv) && fromEnv > 0 ? fromEnv : null;
}

// Espera humana con dos canales (2026-07-14): ENTER local en la terminal O
// una respuesta REMOTA desde el celular (botones en la notificación ntfy →
// POST al tópico de respuestas — ver lib/ntfy.js). Base común de
// pauseForHumanInteraction y confirmToContinue.
//   - 'ok' remoto  → resuelve igual que un ENTER ({ remote: true }).
//   - 'abort' remoto → lanza HumanAbortError (subclase de HumanTimeoutError:
//     todos los callers ya tratan eso como "abandonar esta canción").
//   - deadline vencido en cualquiera de los dos → HumanTimeoutError.
async function waitForHumanResponse({ reason, requestId, timeoutMs, abortSignal = null }) {
  const sinceEpochS = Math.floor(Date.now() / 1000);
  const raceCtl = new AbortController();
  if (abortSignal) {
    if (abortSignal.aborted) raceCtl.abort();
    else abortSignal.addEventListener('abort', () => raceCtl.abort(), { once: true });
  }

  try {
    const result = await Promise.race([
      waitForEnterKey(raceCtl.signal, timeoutMs).then((r) => ({ source: 'local', ...r })),
      waitForNtfyReply({ requestId, sinceEpochS, timeoutMs, abortSignal: raceCtl.signal })
        .then((verb) => ({ source: 'remote', verb })),
    ]);

    if (result.source === 'remote') {
      if (result.verb === 'abort') {
        throw new HumanAbortError(`Abort REMOTO desde el celular: ${reason} — se abandona esta acción/canción de inmediato.`);
      }
      if (result.verb === 'ok') {
        console.log('\n✅ Confirmación REMOTA recibida (botón en la notificación) — continuando.\n');
        return { timedOut: false, remote: true };
      }
      // verb null = el poll remoto llegó a su deadline apenas antes que el
      // timer local — misma semántica que el timeout local de siempre.
      throw new HumanTimeoutError(`Pasaron ${Math.round((timeoutMs || 0) / 60000)} min sin respuesta humana (ni ENTER ni celular) — se abandona esta acción en vez de esperar para siempre.`);
    }
    return { ...result, remote: false };
  } finally {
    // Limpia SIEMPRE el perdedor del race: el listener de stdin (que mantiene
    // vivo el event loop) y el loop de poll remoto.
    raceCtl.abort();
  }
}

// Pausa el script y notifica al usuario para que intervenga de forma manual
// en la UI (Fallback Interactivo). Espera para siempre salvo que
// CANCIONETERNA_HUMAN_TIMEOUT_MS esté seteado en el entorno (--loop) o se
// pase options.timeoutMs explícito — en ese caso, LANZA HumanTimeoutError en
// vez de seguir esperando, para que el caller (siempre dentro de un
// try/catch o del .catch() de nivel de script — ver convención del repo)
// aborte esta canción puntual en vez de trabar la cola entera toda la noche.
// Desde 2026-07-14 la pausa también se puede resolver desde el CELULAR con
// los botones de la notificación (✅ continuar / 🛑 abandonar), y
// options.screenshotPaths adjunta screenshots al aviso para decidir a
// distancia sin ir a la PC.
async function pauseForHumanInteraction(reason, options = {}) {
  ...
  console.error(`Presiona ENTER en esta terminal (o el botón ✅ en la notificación del celular) cuando hayas terminado.`);
  console.error(`======================================================\n`);

  const timeoutMs = options.timeoutMs ?? defaultHumanTimeoutMs();
  const requestId = newRequestId();

  // Screenshots de contexto ANTES del mensaje con botones, para que en el
  // celular se vea primero la evidencia y después la pregunta.
  for (const shot of options.screenshotPaths || []) {
    try {
      if (require('fs').existsSync(shot)) await notifyAttachment(shot, { title: 'Contexto de la pausa' });
    } catch { /* attachment es best-effort, nunca frena la pausa */ }
  }

  await notifyWithReplyActions(
    (timeoutMs
      ? `Interacción manual requerida: ${reason}\n\n⏱️ Si nadie responde en ${Math.round(timeoutMs / 60000)} min, se abandona esta canción y el loop sigue con la próxima.`
      : `Interacción manual requerida: ${reason}`) +
    '\n\nPodés responder con los botones de esta notificación (sin ir a la PC).',
    {
      requestId,
  ...
  try {
    return await waitForHumanResponse({ reason, requestId, timeoutMs, abortSignal: options.abortSignal });
  } catch (e) {
    if (e instanceof HumanAbortError) {
      console.error(`\n🛑 ${e.message}\n`);
      await notify(`🛑 Abort remoto confirmado: ${reason}\nSe abandona esta acción/canción y el loop sigue.`, {
        title: '🛑 Abandonada por decisión remota',
        priority: 'high',
        tags: 'no_entry'
      }).catch(() => {});
    } else if (e instanceof HumanTimeoutError) {
      console.error(`\n⏱️ ${e.message}\n`);
      await notify(`⏱️ Nadie respondió a tiempo: ${reason}\nSe abandona esta acción/canción y el loop sigue.`, {
        title: '⏱️ Timeout de interacción manual',
        priority: 'urgent',
        tags: 'alarm_clock'
      }).catch(() => {});
    }
...
async function confirmToContinue(summary, { nextAction = 'continuar', abortSignal = null, timeoutMs = null, screenshotPaths = [] } = {}) {
  ...
  console.log('Presioná ENTER en esta terminal (o el botón ✅ del celular) para continuar.');
  console.log('──────────────────────────────────────────────────────\n');

  const effectiveTimeoutMs = timeoutMs ?? defaultHumanTimeoutMs();
  const requestId = newRequestId();

  for (const shot of screenshotPaths) {
    try {
      if (require('fs').existsSync(shot)) await notifyAttachment(shot, { title: 'Contexto del checkpoint' });
    } catch { /* best-effort */ }
  }

  await notifyWithReplyActions(
    `${summary}\n\n➡️ Al confirmar: ${nextAction}\n(Respondé con los botones o con ENTER en la terminal)`,
    {
      requestId,
      ...
      tags: 'hand',
      verbs: [
        { verb: 'ok', label: `✅ ${nextAction.slice(0, 40)}` },
        { verb: 'abort', label: '🛑 Abortar' },
      ],
    }
  );

  await waitForHumanResponse({ reason: nextAction, requestId, timeoutMs: effectiveTimeoutMs, abortSignal });
  console.log('✅ Confirmado — continuando.\n');
}

module.exports = { safeClick, identifyBlocker, isClickable, clickByText, setSliderValue, expandIfCollapsed, withReloadRetry, connectToSunoTab, connectToFlowTab, isLoggedIn, isPortUp, ensurePortIsFree, pauseForHumanInteraction, confirmToContinue, waitForEnterKey, waitForHumanResponse, HumanTimeoutError, HumanAbortError, defaultHumanTimeoutMs };
