...
function hardValidate(fullResponse, surveyText) {
  ...
  for (const name of firstNames) {
    ...
    if (canonical && !nameRegex(canonical).test(lyricsOnly)) {
      ...
      // vez de "María"). Ese caso ya lo detecta y reporta H2 arriba (Eñe/
      // tilde perdida), que SÍ es patcheable por el corrector barato (ver
      // isSafeToPatch). Reportarlo también acá como fallo NO patcheable
      // bloqueaba el parche barato para el 100% de los casos (el fallo de M
      // no está en PATCHABLE_FAILURE_PREFIXES a propósito, porque cubre
      // respellings genuinos que no son un simple typo) y forzaba un regen
      // completo caro que no lo arreglaba de forma confiable (bug real
      // 2026-07-13, "El Lago Donde Aprendí a Quedarme": 3 intentos seguidos
      // sin corregir "maria"→"María"). Si ya existe la forma sin acentuar,
      // es el mismo typo que H2 — no duplicar el fallo.
...
// song-corrector.js) para hacer un reemplazo de texto directo. Preserva
// mayúscula inicial (Maria->María, no maria->María) usando el mismo límite
// de palabra consciente del español que el resto del validador (nameRegex).
// Solo toca palabras con suggestions.length === 1 — cualquier ambigüedad
// (2-3 variantes posibles) se deja para el corrector de Haiku o el regen
// completo, igual que antes.
function applyDeterministicAccentFixes(letras) {
  let appliedCount = 0;
  const fixedLetras = {};
    ...
    fixedLetras[sec] = (lines || []).map((line) => {
      ...
      for (const { word, suggestions } of findAccentTypos(line)) {
        ...
        newLine = newLine.replace(nameRegex(word), (match) => {
          appliedCount++;
          return match[0] === match[0].toUpperCase() ? capitalize(suggestion) : suggestion;
        });
      }
      return newLine;
    });
...
module.exports = { parseSections, hardValidate, validateContentForWrite, extractField, convertJsonToMarkdown, isSafeToPatch, applyDeterministicAccentFixes };
