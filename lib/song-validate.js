...
function hardValidate(fullResponse, surveyText) {
  ...
  if (firstNames.length > 0) {
    ...
    if (isMultiRecipient) {
      ...
      // \b nativo de JS no trata á/é/í/ó/ú/ñ como caracteres de palabra (mismo
      // bug ya documentado más abajo para el chequeo de trato) — un nombre que
      // TERMINA en vocal acentuada (José, René) nunca matchea con \b porque la
      // transición palabra→no-palabra ya "se gastó" en el carácter acentuado
      // antes de llegar al límite real de la palabra. Mismo fix: lookbehind/
      // lookahead negativos contra el alfabeto español en vez de \b.
      const nameRegex = (name) => new RegExp(`(?<![a-záéíóúñ])${escapeRegex(name)}(?![a-záéíóúñ])`, 'i');
      const nameInSection = (sec, name) => (sections[sec] || []).some((line) => nameRegex(name).test(line));
  ...
  const trato = (parsedJson.trato || '').toLowerCase();
  if (trato === 'usted') {
