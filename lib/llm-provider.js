...
const QA_CHECKLIST_KEYS = [
  ...
  'eventos_similares_desambiguados',
    ...
    try {
      if (provider === 'gemini') {
        ...
        const response = await fetch('https://api.anthropic.com/v1/messages', {
          method: 'POST',
          // Timeout HTTP propio (2026-07-14): sin signal, un socket colgado de
          // la API dejaba la corrida trabada hasta el techo de etapa de 25 min
          // + kill del watchdog. 120s cubre de sobra la generación real
          // (~30-60s con 8-16k tokens) y convierte el cuelgue en un error
          // reintentable por el backoff de abajo.
