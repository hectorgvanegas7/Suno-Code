...
function applyPhoneticReplacements(lyrics) {
  ...
  const dictPath = path.join(__dirname, 'name-dictionary.json');
  if (!fs.existsSync(dictPath)) return lyrics;
  try {
    const dict = JSON.parse(fs.readFileSync(dictPath, 'utf-8'));
    let output = lyrics;
    for (const [orig, phon] of Object.entries(dict)) {
      const regex = new RegExp(`\\b${orig}\\b`, 'gi');
      output = output.replace(regex, (match) => {
        const isCapitalized = match[0] === match[0].toUpperCase();
        if (isCapitalized) {
          return phon[0].toUpperCase() + phon.slice(1);
        } else {
          return phon.toLowerCase();
        }
      });
    }
    return output;
  } catch (e) {
    console.warn('⚠️ No se pudo aplicar el diccionario fonético:', e.message);
    return lyrics;
  }
}

module.exports = { parseSongFile, buildFlowNotes, buildRedoAwareNotes, applyPhoneticReplacements };
