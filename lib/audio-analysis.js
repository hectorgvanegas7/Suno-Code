...
// (probabilidad baja) cuenta como ausente, para que el auto-reroll lo agarre.
function isNameInTranscription(name, transcribedText, segments = []) {
  ...
  if (allWords.length > 0) {
    ...
    if (bestScore > 0.8 && bestMatch) {
      return bestMatch.probability >= 0.65;
    }
    return false;
  }
  ...
  try {
    ...
    if (transcribedText) {
      ...
      if (firstNames && firstNames.length > 0) {
        const segments = report.transcription?.segments || [];
        report.missingNames = firstNames.filter((name) => !isNameInTranscription(name, transcribedText, segments));
      }
    }
