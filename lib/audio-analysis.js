...
// Similitud 0..1 basada en cobertura de palabras (N-grams de tamaño 3).
// Inmune a repeticiones de coros o estrofas desordenadas. Ideal para canto.
function calculateLyricsCoverage(lyrics, transcription) {
  const normL = normalizeForCompare(lyrics).split(/\s+/).filter(w => w);
  const normT = normalizeForCompare(transcription).split(/\s+/).filter(w => w);
  if (normL.length === 0 || normT.length === 0) return 0;

  // Tamaño del n-grama (3 palabras consecutivas)
  const N = 3;
  if (normL.length < N) {
    // Si la letra es cortísima, caer en Levenshtein normal
    return levenshteinSimilarity(lyrics, transcription);
  }

  // Generar n-gramas de la transcripción para búsqueda rápida
  const transNgrams = new Set();
  for (let i = 0; i <= normT.length - N; i++) {
    transNgrams.add(normT.slice(i, i + N).join(' '));
  }

  // Contar cuántos n-gramas de la letra original existen en la transcripción
  let matchCount = 0;
  let totalNgrams = 0;
  for (let i = 0; i <= normL.length - N; i++) {
    totalNgrams++;
    const lyricNgram = normL.slice(i, i + N).join(' ');
    // Búsqueda difusa ligera: si el n-gram exacto no está, intentamos match difuso
    // (Whisper suele equivocarse en una vocal o consonante)
    if (transNgrams.has(lyricNgram)) {
      matchCount++;
    } else {
      // Intentar match con Levenshtein > 0.8 contra el pool (costoso pero letra es corta)
      let found = false;
      for (const tgram of transNgrams) {
        const dist = distance(lyricNgram, tgram);
        if ((1 - dist / Math.max(lyricNgram.length, tgram.length)) > 0.8) {
          found = true;
          break;
        }
      }
