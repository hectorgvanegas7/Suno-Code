...
async function detectTruncatedWords(mp3Path, segments, { clipDir = null } = {}) {
  ...
  for (const c of candidates) {
    ...
    results.push({
      ...c,
      volumeDropDb,
      volumeDropConfirmed: volumeDropDb !== null && volumeDropDb > TRUNCATION_TAIL_DROP_DB,
      clipPath,
    });
  }
  return results;
...
async function analyzeAudio(mp3Path, { label = 'Versión', titulo = '', lyricsText = '', useDemucs = false, duration = undefined, firstNames = [], prepared = null, transcriptionOutcome = null, clapOutcome = null, nisqaOutcome = null, expectedGender = null, f0Outcome = null } = {}) {
  const report = {
    label,
    ...
    f0Gender: { medianF0Hz: null, voicedRatio: null, detectedGender: null, expectedGender: expectedGender || null, mismatch: null, error: null },
    demucs: { used: false, vocalPresence: null, error: null },
    timing: { demucsMs: null, whisperMs: null, clapMs: null, nisqaMs: null, f0Ms: null, totalMs: null },
    summary: '',
  };
  ...
  try {
    ...
    if (f0Outcome) {
      ...
      const f0Path = prep.demucs.used ? prep.targetPath : mp3Path;
      const f0Result = runF0GenderCheck([f0Path]);
      const fr = f0Result.results[0];
      if (fr.error) {
  ...
  if (report.f0Gender?.detectedGender) {
    ...
    if (report.f0Gender.mismatch === true) {
