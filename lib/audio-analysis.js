...
// "MUY fuera de rango" (caso real 2026-07-14, "El Hombre De Mi Vida": ambas
// versiones salieron 5:26-5:36 con versos repetidos/loop — casi el doble del
// techo). Distinto de durationOk (rango fino, informativo): esto marca una
// generación probablemente ROTA de Suno, y lo usan verify-audio.js (aviso
// temprano apenas ffprobe responde, sin esperar los ~6 min de Whisper/demucs)
// y start-flow.js (pausa para revisión humana apenas termina la descarga).
// Margen 1.5x para no disparar con canciones apenas largas/cortas (3:31-5:14
// NO es "wildly off", solo el rango fino de arriba las marca).
const DURATION_WILD_MARGIN = 1.5;
function isDurationWildlyOff(durationS) {
  return durationS != null && Number.isFinite(durationS)
    && (durationS > MAX_DURATION_S * DURATION_WILD_MARGIN || durationS < MIN_DURATION_S / DURATION_WILD_MARGIN);
}
...
// Similitud 0..1 basada en Cobertura de Palabras (Unigramas Difusos).
// Verifica qué porcentaje de las palabras de la letra original fueron
// cantadas por Suno (y transcritas por Whisper), ignorando el orden
// y sin penalizar texto extra (coros repetidos, alucinaciones).
function calculateLyricsCoverage(lyrics, transcription) {
  const normL = normalizeForCompare(lyrics).split(/\s+/).filter(w => w.length > 2); // ignorar conectores cortos
  const normT = normalizeForCompare(transcription).split(/\s+/).filter(w => w.length > 2);
  if (normL.length === 0 || normT.length === 0) return 0;

  let matchCount = 0;
  // Usar un array clonado para ir tachando palabras encontradas y no re-usarlas
  const tWords = [...normT];

  for (const lWord of normL) {
    let foundIndex = -1;
    ...
    for (let i = 0; i < tWords.length; i++) {
      ...
      if (tWord === lWord) {
        foundIndex = i;
        break; // Match exacto, salir rápido
      }
      
      // Match difuso (permite 1 letra distinta por cada 4 letras)
      const d = distance(lWord, tWord);
      const maxDist = Math.floor(Math.max(lWord.length, tWord.length) / 4);
    ...
    if (foundIndex !== -1) {
      matchCount++;
      // Tachar la palabra encontrada para que no cuente doble
      tWords.splice(foundIndex, 1);
    }
  }

  return matchCount / normL.length;
}
...
async function analyzeAudio(mp3Path, { label = 'Versión', titulo = '', lyricsText = '', useDemucs = false, duration = undefined, firstNames = [], prepared = null, transcriptionOutcome = null, clapOutcome = null, nisqaOutcome = null, expectedGender = null, f0Outcome = null, muqOutcome = null, audioboxOutcome = null } = {}) {
  ...
  try {
    ...
    if (transcribedText && cleanLyrics) {
      ...
      report.levenshteinScore = calculateLyricsCoverage(cleanLyrics, transcribedText);
