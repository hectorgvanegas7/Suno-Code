...
function runNisqaProcess(pythonArgs, resultCount) {
  const outcome = {
...
function runNisqaScore(audioPaths, { device = null } = {}) {
  ...
  if (device) pythonArgs.push('--device', device);
  return runNisqaProcess(pythonArgs, audioPaths.length);
}
...
async function analyzeAudio(mp3Path, { label = 'Versión', titulo = '', lyricsText = '', useDemucs = false, duration = undefined, firstNames = [], prepared = null, transcriptionOutcome = null, clapOutcome = null, nisqaOutcome = null } = {}) {
  const report = {
    label,
    ...
    demucs: { used: false, vocalPresence: null, error: null },
    timing: { demucsMs: null, whisperMs: null, clapMs: null, nisqaMs: null, totalMs: null },
    summary: '',
  };
