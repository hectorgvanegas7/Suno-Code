...
// Retención de MP3s (2026-07-14, pedido de Hector: el SSD se llenaba —
// Downloads/suno/ pesaba 824M, mp3/ 367M). Dos retenciones DISTINTAS a
// propósito:
//   - Downloads/suno/ (SUNO_WORKING_RETENTION_DAYS, 7 días): son las copias
//     de TRABAJO (versión A/B recién descargada) — en cuanto la canción se
//     sube al Flow, la única copia que importa de acá en más es el respaldo
//     de mp3/. Retención corta a propósito, es pura redundancia efímera.
//   - mp3/ (RETENTION_DAYS, 30 días, el mismo default de siempre): es el
//     ÚNICO respaldo archivado con Song ID de una canción ya ENTREGADA a un
//     cliente real — si algún día hace falta reenviarla, esa es la fuente.
//     30 días de margen para cualquier reclamo antes de borrarse solo.
const SUNO_WORKING_RETENTION_DAYS = 7;
const SUNO_WORKING_RETENTION_MS = SUNO_WORKING_RETENTION_DAYS * 24 * 60 * 60 * 1000;
...
// crecían sin rotación, auditoría 2026-07-09), más los MP3s de trabajo en
// Downloads/suno/ (7 días) y el respaldo archivado en mp3/ (30 días —
// 2026-07-14, ver comentario de las constantes arriba). Nunca lanza —
// cualquier error queda en el resultado, no interrumpe al caller.
function rotateOldRunFiles({ repoRoot = path.join(__dirname, '..'), now = Date.now() } = {}) {
  ...
  const cleanIfExists = (dir, opts) => (fs.existsSync(dir) ? cleanOldFiles(dir, { now, ...opts }) : emptyResult);
  ...
  const targets = {
    ...
    // Solo archivos EN LA RAÍZ de SUNO_DIR (los .mp3 de trabajo) — cleanOldFiles
    // nunca baja a subcarpetas, así que name-check/ y truncated-words/ (con su
    // propia retención de 30 días arriba) quedan intactos acá.
    sunoWorkingMp3s: cleanIfExists(SUNO_DIR, { retentionMs: SUNO_WORKING_RETENTION_MS }),
    mp3Backup: cleanOldFiles(path.join(repoRoot, 'mp3'), { now }),
  ...
  const totalDeleted = targets.logs.deleted.length + targets.screenshots.deleted.length
    ...
    + targets.sunoWorkingMp3s.deleted.length + targets.mp3Backup.deleted.length;
  if (totalDeleted > 0) {
    console.log(`🧹 Higiene: borrados ${totalDeleted} archivo(s) viejo(s) (logs/screenshots +${RETENTION_DAYS}d, clips de audio +${RETENTION_DAYS}d, Downloads/suno +${SUNO_WORKING_RETENTION_DAYS}d, mp3/ +${RETENTION_DAYS}d).`);
    if (targets.sunoWorkingMp3s.deleted.length > 0) console.log(`   Downloads/suno/: ${targets.sunoWorkingMp3s.deleted.length} MP3(s) de trabajo (+${SUNO_WORKING_RETENTION_DAYS}d)`);
    if (targets.mp3Backup.deleted.length > 0) console.log(`   mp3/: ${targets.mp3Backup.deleted.length} respaldo(s) archivado(s) (+${RETENTION_DAYS}d)`);
  }
  ...
  const allErrors = [...targets.logs.errors, ...targets.screenshots.errors, ...targets.nameCheckClips.errors, ...targets.truncatedClips.errors, ...targets.sunoWorkingMp3s.errors, ...targets.mp3Backup.errors, ...targets.jsonlTrim.errors];
...
module.exports = { cleanOldFiles, trimGrowingJsonlFiles, rotateOldRunFiles, RETENTION_DAYS, MAX_JSONL_LINES, SUNO_WORKING_RETENTION_DAYS };
