...
// respalda "trece de mayo"). Regla de flag equilibrada para señal informativa:
//   - cualquier token CAPITALIZADO del hecho ausente de la encuesta → flag
//     ("Miami"), salvo whitelist religiosa (regla 8) / respellings.
//   - o TODOS sus tokens significativos ausentes → flag ("un martes de
//     octubre" inventado entero), pero un hecho parcialmente respaldado
//     ("el hogar que construimos" con "hogar" en la encuesta) pasa.
const HECHOS_STOPWORDS = new Set(['de', 'del', 'la', 'el', 'los', 'las', 'un', 'una', 'unos', 'unas', 'en', 'y', 'a', 'que', 'mi', 'tu', 'su', 'sus', 'por', 'con', 'al', 'se', 'lo', 'era', 'fue', 'nos', 'cuando']);
function compararHechosConEncuesta(hechos, surveyText, { firstNames = [] } = {}) {
  ...
  const evaluar = (tipo, valor) => {
    ...
    const significativos = rawTokens.filter((t) => t.length >= 3 && !HECHOS_STOPWORDS.has(norm(t)) && !MIDLINE_PROPER_NOUN_WHITELIST.has(norm(t)));
    ...
    if (significativos.length > 0 && significativos.every((t) => !surveyTokens.has(norm(t)))) {
      return { tipo, valor, motivo: 'ningún término del hecho aparece en la encuesta' };
    }
    return null;
  };
