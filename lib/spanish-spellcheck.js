...
//
// DOS niveles (2026-07-13, tras el corrector determinístico sin LLM de
// song-validate.js): cuando este blocklist se escribió, el costo de un
// falso positivo era "el corrector de Haiku revisa la línea con contexto".
// applyDeterministicAccentFixes cambió ese costo a "reemplazo directo
// silencioso" — y hay entradas donde la forma sin tilde es una palabra
// perfectamente plausible en una letra ("El Papa nos bendijo" —> "El Papá",
// "yo sueno como campana" —> "yo sueño"; verificado en vivo 2026-07-13).
// Esas van en el nivel CONTEXT: se siguen marcando como fallo patcheable
// (H2 de hardValidate), pero SOLO las arregla el corrector de Haiku, que ve
// la línea completa y puede juzgar — nunca el reemplazo determinístico.
const ENYE_TYPOS_BLOCKLIST = {
  ...
  mama: 'mamá', mamas: 'mamás', // "mama" (verbo/pecho) es rarísimo en dedicatorias
  jamas: 'jamás',
  ademas: 'además',
  ultimo: 'último', ultima: 'última',
  publico: 'público', // "yo publico" (publicar) ~imposible en una letra dedicada
  medico: 'médico',
};
// Homógrafos donde la forma sin tilde es PLAUSIBLE en una letra — marcar sí,
// auto-reemplazar jamás (needsContext: true en el resultado).
const ENYE_TYPOS_BLOCKLIST_CONTEXT = {
  papa: 'papá', papas: 'papás', // "el Papa" es real en canciones cristianas
  sueno: 'sueño', suenos: 'sueños', // "yo sueno" (sonar) existe
};
...
// que probablemente le falta una tilde o una eñe — con `needsContext: true`
// cuando la palabra viene del blocklist de homógrafos plausibles (solo un
// corrector CON contexto puede decidir; el determinístico debe saltearla).
// `text` es la letra cruda (todas las secciones concatenadas). No mira
// nombres propios — el caller (hardValidate) ya filtra esos con
// canonicalStandardSpanishName antes de llegar acá si hace falta, pero esta
// función igual es conservadora: una palabra sin NINGUNA variante válida en
// el diccionario (caso típico de un nombre propio o respelling fonético)
// nunca se marca.
function findAccentTypos(text) {
  ...
  for (const raw of words) {
    ...
    if (ENYE_TYPOS_BLOCKLIST_CONTEXT[word]) {
      found.push({ word, suggestions: [ENYE_TYPOS_BLOCKLIST_CONTEXT[word]], needsContext: true });
      continue;
    }
