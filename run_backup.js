...
- [ ] Suno style prompt ends with "Latin American Spanish, neutral accent, seseo"?
...
**Trato:** [tú / usted / vos]
**Estilo Suno:** [style prompt — always ending with "Latin American Spanish, neutral accent, seseo"]
...
function checklistFailures(fullResponse) {
  if (/⚠️\s*REVISAR MANUALMENTE/i.test(fullResponse)) {
    return ['- Claude marcó la letra como ⚠️ REVISAR MANUALMENTE en su propia respuesta.'];
  }

  const checklistIndex = fullResponse.search(/\*\*QA Checklist:\*\*/i);
  if (checklistIndex === -1) {
    return ['No se encontró el bloque "QA Checklist" en la respuesta.'];
  }
...
// Frases sin sentido lógico ya observadas en producción. Sirve como red de
// seguridad además del autochequeo que hace Claude en su propio checklist.
const KNOWN_INCOHERENT_PHRASES = ['nunca hemos estado difuntos', 'padre de mi mano'];

// Marcadores de tú/vos que nunca deberían aparecer cuando el trato declarado
// es "usted" (incluye formas imperativas, que es donde se detectaron las
// mezclas reales en corridas anteriores).
const USTED_MISMATCH_MARKERS = [
  /\bno tardes\b/i,
  /\bmir[áa]\b/i,
  /\bvení\b/i,
  /\bdecí\b/i,
...
function validateAndFixLyrics(fullResponse) {
  const failures = [...checklistFailures(fullResponse)];

  const lowerResponse = fullResponse.toLowerCase();
  for (const phrase of KNOWN_INCOHERENT_PHRASES) {
    if (lowerResponse.includes(phrase)) {
      failures.push(`- Frase incoherente detectada: "${phrase}"`);
    }
  }

  const trato = (extractField(fullResponse, 'Trato') || '').toLowerCase();
  if (trato === 'usted') {
    for (const marker of USTED_MISMATCH_MARKERS) {
      const match = fullResponse.match(marker);
      if (match) {
        failures.push(`- Mezcla de trato detectada ("${match[0]}") pese a que el Trato declarado es "usted"`);
      }
    }
  }
...
async function generateSongWithSelfCorrection(surveyContent) {
  ...
  for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
    console.log(`Generando letra con Claude (intento ${attempt}/${MAX_GENERATION_ATTEMPTS})...`);
    lastResponse = await generateSongWithClaude(userMessage);
    const { valid, failures } = validateAndFixLyrics(lastResponse);
    lastFailures = failures;

    if (valid) {
      console.log('QA Checklist: todos los ítems pasaron.');
      return { fullResponse: lastResponse, passedQA: true };
    }

    console.log(`QA Checklist encontró problemas en el intento ${attempt}:`);
    lastFailures.forEach((line) => console.log(`  ${line.trim()}`));

    const correctiveNotes = [];
    if (lastFailures.some((f) => /trato/i.test(f))) {
      const trato = extractField(lastResponse, 'Trato') || 'el trato indicado en la encuesta';
      correctiveNotes.push(
        `INSTRUCCIÓN EXPLÍCITA: mantené el trato "${trato}" sin excepción en TODA la letra, incluyendo el Outro y cualquier frase imperativa (ej. "no tarde" para usted, nunca "no tardes", "no tardés", "mirá" ni ninguna otra forma de tú/vos).`
      );
    }
    if (lastFailures.some((f) => /incoherente/i.test(f))) {
      correctiveNotes.push(
        'INSTRUCCIÓN EXPLÍCITA: revisá cada línea y reescribí cualquier frase sin sentido lógico o semántico dentro del contexto de la canción. Cada línea debe tener un significado claro.'
      );
    }

    if (correctiveNotes.length > 0) {
      userMessage = `${baseUserMessage}\n\n${correctiveNotes.join('\n')}`;
    }
  }

  console.log(`No se logró pasar el checklist QA después de ${MAX_GENERATION_ATTEMPTS} intentos. Se guardará con advertencia.`);
...
(async () => {
  ...
  try {
    ...
    const warningBanner = passedQA
      ? ''
      : `⚠️ ADVERTENCIA: no pasó el checklist QA después de ${MAX_GENERATION_ATTEMPTS} intentos. Revisar manualmente.\n\n`;

    const songContent = `${warningBanner}${lyricsContent}\n\n${notesLine}`;

    fs.writeFileSync(SONG_PATH, songContent, 'utf-8');
    console.log(`Canción guardada en ${SONG_PATH}`);
    ...
    console.log(
      passedQA
        ...
        : '⚠️ Listo, pero con advertencia QA. Revisá song.txt cuidadosamente antes de continuar.'
    );
  } finally {
    await context.close();
  }
